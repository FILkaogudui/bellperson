    ...
    E: Engine,
{
        ...
        G: CurveAffine,
    {
        ...
        let mem1 = size1 * n;
        let mem2 = size2 * n;
        let mut mem3 = size3 * 2 * self.core_count * bucket_len;
        let mem4 = size3 * 2 * self.core_count;

        let gate = (self.program.device().memory() * 95 / 100) as usize;
        while(mem1 + mem2 + mem3 + mem4) > gate && gate > (mem1 + mem2 + mem4) {
            window_size = window_size - 1;
            num_windows = ((exp_bits as f64) / (window_size as f64)).ceil() as usize;
            num_groups = calc_num_groups(self.core_count, num_windows);
            bucket_len = 1 << window_size;

            mem3 = size3 * 2 * self.core_count * bucket_len;
            info!("GATELOG after gate = {:?} window_size = {:?} mem3 = {:?}",gate, window_size, mem3);
        }
